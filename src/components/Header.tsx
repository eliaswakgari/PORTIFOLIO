...
const Header: React.FC = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Changed to show white background immediately (scrollY > 0)
  ...
  return (
    <>
      {/* Main Header */}
      <motion.header
        className={`fixed top-0 left-0 w-screen z-50 transition-all duration-500 ease-out h-[10vh] min-h-[80px] flex items-center ${
          isScrolled 
      ...
      >
        <div className="w-full max-w-[100vw] mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between w-full">
            {/* Logo - Moved to left with margin */}
            <motion.div
            ...
            {/* Desktop Navigation - Flex with justify-around and margin from right */}
            <nav className="hidden lg:flex items-center justify-around flex-1 max-w-2xl mr-8 lg:mr-12">
                ...
                >
                  ...
                  >
                    <span className="relative">
                      {item.name}
                      <motion.span
                        className={`absolute -bottom-1 left-0 rounded-full bg-gradient-to-r from-purple-600 to-pink-600 transition-all duration-300 ${
                          activeSection === item.href.slice(1) ? 'w-full h-1' : 'w-0 h-0.5 group-hover:w-full'
                        }`}
            ...
            {/* Hire Me Button - Positioned at right corner with margin */}
            <motion.div
              className="hidden lg:flex items-center mr-6 lg:mr-8"
            ...
            >
              <motion.a
                href="#contact"
                className={`px-8 py-3 font-bold rounded-2xl shadow-2xl transition-all duration-300 group ${
                  isScrolled
                    ? 'bg-gradient-to-r from-purple-600 to-pink-600 text-white shadow-purple-500/40 hover:shadow-purple-500/60'
                    : 'bg-gradient-to-r from-purple-600 to-pink-600 text-white shadow-purple-500/40 hover:shadow-purple-500/60'
                }`}
                onClick={(e) => {
                  e.preventDefault();
            ...
            {/* Mobile Menu Button */}
            <motion.button
              className={`lg:hidden p-3 rounded-xl transition-all duration-300 mr-4 ${
                isScrolled 
              ...
              <nav className="p-6 space-y-3">
                {navItems.map((item, index) => (
                  <motion.a
                    ...
                    href={item.href}
                    className={`
                      flex items-center justify-between w-full p-4 rounded-xl font-semibold transition-all duration-300 group
                      ${activeSection === item.href.slice(1) 
                  ...
                  >
                    ...
                    <motion.div
                      className={`w-2 h-2 rounded-full bg-gradient-to-r from-purple-600 to-pink-600 ${
                        activeSection === item.href.slice(1) ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'
